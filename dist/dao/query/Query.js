...
class Query {
    ...
    hitList(list) {
        return ArrayUtil_1.ArrayUtil.filter(list, (row) => this.isHit(row));
    }
    /**
     * 查询 肯定没结果
     * in一个空数组
    ...
    */
    ...
    sql(sql, val) {
        this.addCdt(new SqlCdt_1.default(sql, val));
        return this;
    }
    /**
     * 增加 不等于 查询
    ...
    */
     ...
     * @param params
     */
    static parseJson(json, ...params) {
        let query = Query.parse(BeanUtil_1.BeanUtil.parseJsonFromParam(json.query, ...params));
        if (json.cdts) {
            for (let cdt of json.cdts) {
                let val = BeanUtil_1.BeanUtil.changeVal(cdt.value, ...params);
                if (val != null)
                    query.addCdt(new Cdt_1.default(cdt.col, val, cdt.op));
            }
        }
        if (json.having) {
            for (let cdt of json.having) {
                let val = BeanUtil_1.BeanUtil.changeVal(cdt.value, ...params);
                if (val != null)
                    query.addHaving(new Cdt_1.default(cdt.col, cdt.value, cdt.op));
            }
        }
        if (json.group) {
...
const SqlCdt_1 = __importDefault(require("./cdt/imp/SqlCdt"));
