...
class FindByIdSql extends SqlBuilder_1.default {
    build(data, opts) {
        ...
        var ids = opt.acqIds();
        if (ids.length == 1) {
            let pojoId = this.parseDbField(ids[0]);
            this._pushSqlTxt(sql, new sql_1.ColSql(ids[0]));
            this._pushSqlTxt(sql, '=');
            this._pushSqlTxt(sql, new sql_1.ValSql(typeof data === 'object' ? data[pojoId] : data));
        }
            ...
            for (let i = 0; i < ids.length; i++) {
                ...
                this._pushSqlTxt(sql, '=');
                this._pushSqlTxt(sql, new sql_1.ValSql(data[this.parseDbField(name)]));
            }
        }
        return sql;
    }
