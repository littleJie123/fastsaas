...
class TimezoneServer {
    ...
    getDate(date) {
        ...
        if (date == null && tmpCtx != null && tmpCtx.getData('now') != null) {
    ...
    sysToTzDate(timezone, sysDate) {
        ...
        return new Date(sysDate.getTime() + (sysOffset + (0, TimezoneUtil_1.getUtcOffset)(timezone, sysDate.getTime())) * 60000);
    }
    //某个时区转系统
    tzToSysDate(date, timezone) {
        ...
        timezone = this._parseTimezone(timezone);
        return new Date(date.getTime() - (sysOffset + (0, TimezoneUtil_1.getUtcOffset)(timezone, date.getTime())) * 60000);
    }
    /**
     * 由于存的都是时区别名，做一层转换。
     */
